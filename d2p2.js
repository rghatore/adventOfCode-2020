...
const validPasswords = (passwords) => {
  ...
  // check validity
  const valid = passwords.filter((passwordString) => {
    const passwordArray = passwordString.split(" ");
    const range = passwordArray[0].split("-");
    const letter = passwordArray[1][0];
    const password = passwordArray[2];
    // console.log("array: ", passwordArray);
    // console.log("range: ", range);
    // console.log("letter: ", letter);

    // check if the letter shows up at exactly one of the index
    if (password[Number(range[0]) - 1] === letter && password[Number(range[1]) - 1] === letter) {
      return false
    } else if (password[Number(range[0]) - 1] === letter || password[Number(range[1]) - 1] === letter) {
      return true
    } else {
      return false
    }
