...
const validPasswords = (passwords) => {
  ...
  const valid = passwords.filter((passwordString) => {
    ...
    // check how many times the letter at [1] shows up
    const numOfTimes = password.match(regex);
    // console.log("times: ", numOfTimes);
    if (!numOfTimes) {
      return false
    } else if (numOfTimes.length >= Number(range[0]) && numOfTimes.length <= Number(range[1])) {
      return true
    } else {
      return false
    }
